...
class TagsBatchDeleteAPIView(APIView):
    def post(self, request):
        ids = request.data
        try:
            Tags.objects.filter(id__in=ids).delete()
            return SuccessResponse(msg="删除成功")
        except:
            return ErrorResponse(msg="删除失败")


class TagsExport(APIView):
    model = Tags
    ...
    def get(self, request, *args, **kwargs):
        ...
        model = apps.get_model('business','Tags')
