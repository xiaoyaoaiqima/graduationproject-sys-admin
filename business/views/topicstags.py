...
# 分页
class TopicstagsPageView(APIView):
...
class TopicstagsBatchDeleteAPIView(APIView):
    def post(self, request):
        ids = request.data
        try:
            Topicstags.objects.filter(id__in=ids).delete()
            return SuccessResponse(msg="删除成功")
        except:
            return ErrorResponse(msg="删除失败")


class TopicstagsExport(APIView):
    model = Topicstags
    queryset = model.objects.all()
    serializer_class = TopicstagsSerializer

    def get(self, request, *args, **kwargs):
        response = HttpResponse(content_type='application/ms-excel')
        response['Content-Disposition'] = 'attachment; filename="Topicstags.xlsx"'

        # 创建Excel工作簿和工作表
        wb = Workbook()
        sheet = wb.active

        model = apps.get_model('business','Topicstags')
